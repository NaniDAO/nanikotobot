...
export const getHistory = (count?: number): Message[] => {
  try {
    const currentHistory = getHistoryFromFile().map((message) => ({
      ...message,
    }));
...
const extractUsernameAndContent = (
  content: string
...
export const getHistoricalContext = async ({
  ...
  try {
    ...
    context = await queryNaniMemory(query);
    return context;
    } catch (e) {
...
export const queryNaniMemory = async (query: string): Promise<string> => {
  ...
  if (res.status.error_code != "Success") {
    ...
    matches = matches.filter((match) =>
      isRelevantContent(keywords, match.content)
    );
...
const sortMatches = (
  ...
  return matches.sort((a, b) => {
    ...
    const weightedSumA = weightedSum(
      scoreWeight,
      timeWeight,
      normalizedScoreA,
      normalizedTimeA
    );
    const weightedSumB = weightedSum(
      scoreWeight,
      timeWeight,
      normalizedScoreB,
      normalizedTimeB
